#!/usr/bin/env node
/**
 * Update I-90 Definition in Forms Registry
 * 
 * This script replaces the old I-90 definition with the new comprehensive one
 */

const fs = require('fs');

console.log('🔄 Updating I-90 definition in forms registry...\n');

const registryPath = 'src/lib/constants/forms-registry.ts';
let registryContent = fs.readFileSync(registryPath, 'utf-8');

// Find the start and end of I90_DEFINITION
const startMarker = 'const I90_DEFINITION: FormDefinition = {';
const startIndex = registryContent.indexOf(startMarker);

if (startIndex === -1) {
  console.error('❌ Could not find I90_DEFINITION in forms registry');
  process.exit(1);
}

// Find the end of the definition (next "const" or "export const")
const afterStart = registryContent.substring(startIndex + startMarker.length);
const endMarker = '\n};';
const endIndex = afterStart.indexOf(endMarker);

if (endIndex === -1) {
  console.error('❌ Could not find end of I90_DEFINITION');
  process.exit(1);
}

const actualEndIndex = startIndex + startMarker.length + endIndex + endMarker.length;

console.log(`📍 Found I90_DEFINITION at line ${registryContent.substring(0, startIndex).split('\n').length}`);
console.log(`📍 Definition ends at line ${registryContent.substring(0, actualEndIndex).split('\n').length}`);

// New I-90 definition
const newI90Definition = `const I90_DEFINITION: FormDefinition = {
  id: "i-90",
  code: "I-90",
...
const updatedContent = 
  registryContent.substring(0, startIndex) + 
  newI90Definition + 
  registryContent.substring(actualEndIndex);

// Write back to file
fs.writeFileSync(registryPath, updatedContent);

console.log('✅ Successfully updated I-90 definition in forms registry!');
console.log('\n📋 Changes made:');
console.log('  • Updated form description');
console.log('  • Updated filing fee from $455 to $540');
console.log('  • Updated service price from $60 to $89');
console.log('  • Reorganized sections for better user experience');
console.log('  • Added comprehensive field options (states, colors, etc.)');
console.log('  • Improved field labels and placeholders');
console.log('  • Added proper field types (tel, email, etc.)');
console.log('\n🎉 I-90 form definition is now up to date!');